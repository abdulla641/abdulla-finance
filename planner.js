/* Monthly Money Plan: October 2026 financial plan starts after the vacation. Vacation: Sep 17-Oct 20, 2026. */
(function(){
  const PLAN_KEY='moneyPlan';
  const START_MONTH='2026-10';
  const VACATION_START='2026-09-17';
  const VACATION_END='2026-10-20';
  const defaultPlan={emergencyPct:40,savingsPct:30,investmentPct:30,emergencyTarget:24000,investmentTotal:0,savingsTotal:0,startMonth:START_MONTH};
  let plan=JSON.parse(localStorage.getItem(PLAN_KEY)||'null')||defaultPlan;
  plan={...defaultPlan,...plan,startMonth:START_MONTH};
  ...
  function emergencySaved(){const goals=JSON.parse(localStorage.getItem('savingsGoals')||'[]');const g=goals.find(x=>String(x.name||'').toLowerCase().includes('emergency'));return g?n(g.saved):0}
  ...
  function ensurePanel(){
    ...
    panel.innerHTML=`<div class="section-heading"><div><h3>🧭 Where Should My Balance Go?</h3><small id="mpSubtitle">Vacation: 17 Sep–20 Oct 2026 · Financial plan starts after vacation.</small></div></div><div class="money-plan-grid"><div class="money-plan-main"><div class="plan-total"><small id="mpAvailableLabel">Available for October plan</small><strong id="mpAvailable">QAR 0.00</strong></div><div class="allocation-row emergency"><div><strong>🛡️ Emergency Fund</strong><small id="mpEmergencyPct">40%</small></div><strong id="mpEmergency">QAR 0.00</strong></div><div class="allocation-row savings"><div><strong>💰 Savings / Goals</strong><small id="mpSavingsPct">30%</small></div><strong id="mpSavings">QAR 0.00</strong></div><div class="allocation-row investment"><div><strong>📈 Investments</strong><small id="mpInvestmentPct">30%</small></div><strong id="mpInvestment">QAR 0.00</strong></div><div class="plan-note" id="mpAdvice"></div></div><div class="money-plan-side"><label>Emergency target<input id="mpTarget" type="number" inputmode="decimal"></label><label>Emergency %<input id="mpEPct" type="number" min="0" max="100"></label><label>Savings %<input id="mpSPct" type="number" min="0" max="100"></label><label>Investment %<input id="mpIPct" type="number" min="0" max="100"></label><button onclick="saveMoneyPlan()">Save Plan</button><small id="mpValidation" class="hint"></small></div></div><div class="plan-progress"><div><span>🛡️ Emergency Fund</span><strong id="mpEmergencyProgress">QAR 0 / QAR 24,000</strong></div><div class="progress"><i id="mpEmergencyBar" style="width:0%"></i></div></div>`;
    const net=home.querySelector('.net-worth-card');net?home.insertBefore(panel,net):home.appendChild(panel);
  }
  window.saveMoneyPlan=function(){const e=n(document.getElementById('mpEPct').value),s=n(document.getElementById('mpSPct').value),i=n(document.getElementById('mpIPct').value),t=n(document.getElementById('mpTarget').value);if(e+s+i!==100){document.getElementById('mpValidation').textContent='Percentages must total exactly 100%.';return}if(t<=0){document.getElementById('mpValidation').textContent='Enter a valid emergency target.';return}plan={...plan,emergencyPct:e,savingsPct:s,investmentPct:i,emergencyTarget:t,startMonth:START_MONTH};savePlan();document.getElementById('mpValidation').textContent='Saved ✓';renderPlan()};
  window.addInvestment=function(amount){plan.investmentTotal=n(plan.investmentTotal)+n(amount);savePlan();renderPlan()};
  function renderPlan(){
    ensurePanel();const key=currentMonth();const vacation=isVacationMonth(key);const stats=getMonthlyStats(key);const available=vacation?0:Math.max(0,stats.income-stats.expense);const e=available*plan.emergencyPct/100,s=available*plan.savingsPct/100,i=available*plan.investmentPct/100,saved=emergencySaved(),target=n(plan.emergencyTarget),pct=target?Math.min(100,saved/target*100):0;const set=(id,v)=>{const x=document.getElementById(id);if(x)x.textContent=v};
    set('mpAvailableLabel',vacation?'✈️ Vacation Mode — no saving/investment allocation':'Available for '+(key==='all'?'financial plan':key+' plan'));set('mpAvailable',m(available));set('mpEmergency',m(e));set('mpSavings',m(s));set('mpInvestment',m(i));set('mpEmergencyPct',plan.emergencyPct+'%');set('mpSavingsPct',plan.savingsPct+'%');set('mpInvestmentPct',plan.investmentPct+'%');set('mpEmergencyProgress',m(saved)+' / '+m(target));const bar=document.getElementById('mpEmergencyBar');if(bar)bar.style.width=pct+'%';
    const advice=document.getElementById('mpAdvice');if(advice){if(vacation)advice.textContent='✈️ Vacation Mode: 17 Sep–20 Oct 2026. Use this period for India vacation expenses and Qatar fixed commitments only. No automatic emergency, savings or investment allocation.';else if(stats.income===0)advice.textContent='Add your salary transaction to get the real monthly recommendation.';else if(available<=0)advice.textContent='No surplus this month. Focus on controlling expenses before investing more.';else if(saved<target)advice.textContent='Priority: keep building your emergency fund until the target is reached.';else advice.textContent='Emergency target reached. You can direct more of the surplus toward long-term investments.'}
