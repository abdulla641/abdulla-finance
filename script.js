...
function drawChart(income,expense,categories){let c=document.getElementById('chart');if(!c||typeof Chart==='undefined')return;if(chart)chart.destroy();chart=new Chart(c,{type:'doughnut',data:{labels:['Income','Expense'],datasets:[{data:[income,expense]}]},options:{responsive:true}});}
function render(){let income=0,expense=0;let categories={};let list=document.getElementById('list');if(list)list.innerHTML='';data.forEach(x=>{x.type==='income'?income+=x.amount:expense+=x.amount;if(x.type==='expense')categories[x.category]=(categories[x.category]||0)+x.amount;if(list){let li=document.createElement('li');li.innerHTML=`${x.name} (${x.category}) ${x.type==='income'?'+':'-'} ${money(x.amount)} <button onclick="removeItem(${x.id})">Delete</button>`;list.appendChild(li);}});document.getElementById('income').textContent=money(income);document.getElementById('expense').textContent=money(expense);document.getElementById('balance').textContent=money(income-expense);document.getElementById('savingTotal').textContent=money(savings);document.getElementById('summary').textContent=money(income-expense);let chartText=document.getElementById('analytics');if(chartText)chartText.textContent=Object.entries(categories).map(x=>x[0]+': '+money(x[1])).join(' | ')||'No expenses yet';drawChart(income,expense,categories);renderCredit();}
render();